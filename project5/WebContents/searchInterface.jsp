    ...
    <body>
        ...
        <div class= "PagePart">    
			...
			<div class="col-md-6 col-md-offset-3">
				...
				%>
					<tr>
					<td><a href="http://<%=serverName%>:<%=httpServerPort%><%=getContextPath%>/item?ItemID=<%=SR[i].getItemId()%>"> <%=SR[i].getItemId()%>  </a> </td>
					<td><a href="http://<%=serverName%>:<%=httpServerPort%><%=getContextPath%>/item?ItemID=<%=SR[i].getItemId()%>"> <%=SR[i].getName()%> </a> </td>
					</tr>
				<%
				}
				%>
				...
				{
				%>
				<a id="Previous Page" href="http://<%=serverName%>:<%=httpServerPort%><%=getContextPath%>/search?q=<%=query%>&numResultsToSkip=<%=numResultsToSkip-numResultsToReturn%>&numResultsToReturn=<%=numResultsToReturn%>">
						Previous Page</a>
				<%
				}
				%>
				...
				{
				%>
				<a id="Next Page" href="http://<%=serverName%>:<%=httpServerPort%><%=getContextPath%>/search?q=<%=query%>&numResultsToSkip=<%=numResultsToSkip+numResultsToReturn%>&numResultsToReturn=<%=numResultsToReturn%>">
							Next Page</a>
				<%
				}
				%>
